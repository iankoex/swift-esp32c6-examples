...
This example demonstrates how to control an addressable RGB LED strip to create a blinking light effect using Swift code that bridges to the ESP-IDF SDK's C APIs. This example is specifically made for the RISC-V MCUs from ESP32 (the Xtensa MCUs are not currently supported by Swift).
...
This demo depends on `espressif/led_strip: "^3.0.0"` for controlling the LED strip hardware. This dependency is automatically managed by ESP-IDF's component system when building the project.
...
- The LED will start blinking (turning on and off every second), and status messages like "Turning LED ON" or "Turning LED OFF" will be printed to the console/serial output.
